...
import { FiSearch, FiFilter, FiX, FiChevronDown, FiShoppingCart } from 'react-icons/fi'
...
const ShopProductCard = ({ product }) => {
  ...
  return (
    <div className="group flex h-full flex-col overflow-hidden rounded-[22px] border border-slate-200 bg-white text-slate-900 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-2xl">
      <Link to={`/product/${product._id}`} className="overflow-hidden bg-white">
        <img
          ...
          alt={product.name}
          className="h-40 w-full object-cover transition-transform duration-500 group-hover:scale-105 sm:h-36"
        />
      </Link>
      ...
      <div className="flex flex-1 flex-col justify-between p-4">
        <div>
          <Link to={`/product/${product._id}`}>
            <h3 className="min-h-[2.75rem] overflow-hidden text-sm font-semibold leading-tight text-slate-900 line-clamp-2">
              {product.name}
            </h3>
          </Link>

          <p className="mt-2 text-xs text-gray-500">{product.brand || 'Mobile'}</p>

          <div className="mt-3 flex items-center justify-between gap-3">
            <span className="text-lg font-bold tracking-tight text-slate-900">Rs. {formatPrice(product.price)}</span>
            <span className={`text-[10px] font-semibold uppercase tracking-[0.3em] ${product.stock > 0 ? 'text-emerald-600' : 'text-rose-500'}`}>
        ...
        <button
          ...
          className="mt-4 inline-flex items-center justify-center gap-2 rounded-2xl bg-orange-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-orange-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
